...
def user_ask(request):
    ...
    if user_ask_form.is_valid():
        user_ask_form.save(commit=True) #ModelForm比Form牛逼，一行代码即可对表进行保存
...
def user_comment(request):
    ...
    if user_comment_form.is_valid():
        ...
        a.comment_course_id = course    #由于前端传来的键course不是一个对象，而是id，则赋值给UserComment表中字段时，要加_id。如果传来的是一个对象，则直接赋给字段名
        a.save()
...
def user_deletelove(request):
    ...
    lovetype = request.GET.get('lovetype','')
    if loveid and lovetype:
        love = UserLove.objects.filter(love_id=int(loveid),love_type=int(lovetype),love_status=True,love_man=request.user)
        if love:
            love[0].love_status = False
            love[0].save()
            return JsonResponse({
