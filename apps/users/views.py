...
#可以将函数式的view改为类。定义自己的类，继承django.views.generic里的View，有get方法就实现get方法，有post方法就实现post方法
...
# def index(request):
#     all_banners = BannerInfo.objects.all().order_by('-add_time')[:5]
#     banner_courses = CourseInfo.objects.filter(is_banner=True)[:3]  #拿3个轮播的图
#     all_courses = CourseInfo.objects.filter(is_banner=False)[:6]
#     all_orgs = OrgInfo.objects.all()[:15]
#     return render(request,'index.html',{
#         'all_banners':all_banners,
        ...
        if user_login_form.is_valid():
            ...
            if user:
                ...
                if user.is_start:
                    ...
                    ret = redirect(url)
                    ret.delete_cookie('url')    #从COOKIES拿键url的值，只拿一次，用delete_cookie删掉。如：未登录状态下点“开始学习”，跳登录页，登录账号后，跳学习视频页面。之后，未登录状态下点“收藏”，跳登录页，登录账号后，避免仍然跳学习视频页面
                    return ret
                else:
...
def user_active(request,code):
    if code:
        ...
        if email_ver_list:
            ...
            #根据传的code，从EmailVerifyCode表拿到对应的邮箱号
            ...
            if user_reset_form.is_valid():
                ...
                if password == password1:
                    ...
                    if email_ver_list:
                        ...
                        if user_list:
                            user = user_list[0]
                            #对找到的用户，修改其密码。如果用user.password(password1)，则密码字符在数据库表是未加密的
                    ...
                    return render(request,'users/password_reset.html',{
                        'msg':'两次密码不一致',
                        #注意还要返回code，不然用户下次输入完密码点提交，就不知道是修改谁的密码了
                        'code':code
                    })
            else:
                return render(request,'users/password_reset.html',{
...
def user_changeimage(request):
    ...
    #如果不指明instance，那么就会被当作创建对象去执行(我们的目的是要修改，不是创建)，而我们只有一个图片，就一定会报错。
